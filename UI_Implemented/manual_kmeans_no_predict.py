...
def apply_standardization(input_dataset):
    ...
    for i in range(input_columns):
        # Add every row value to variable 
        column_values = [row[i] for row in input_dataset]
        # Calculate mean for the column by subtracting sum of values by the size of input dataset
        input_mean[i] = sum(column_values)/input_rows
...
def PCA():
    ...
    standardized_mean = np.mean(standardized_data.T, axis=1)
    ...
    cov_applied = np.cov(centered_vals.T)
    # Find the eigenvalues and eigenvectors of the covariance matrix
    values, vectors = eig(cov_applied)
    # Project the data using the vectors
    pca_vec = vectors.T.dot(centered_vals.T)
    # Flip sign to enforce deterministic output 
    pca_vec = pca_vec.T * -1 
    # Slice the vectors to retain the first two principal components
    pca_data = pca_vec[:, :2]
    pca_kept = 0
    # Iterate over components to get total value of all components kept
    for i in range(2):
        pca_kept = pca_kept+values[i]
    # Calculate the percentage value of the retained components by dividing kept values by total sum of all (num of features)
    retained_value = pca_kept/sum(values)
    pca_loss = "The information retained by 2 components is: ", retained_value, "%"
    print(pca_loss)
...
def kmeans():
    ...
    for i in tf.range(num_iterations):
        ...
        # Call stack() to return the tensor array values as a concatenated tensor 
